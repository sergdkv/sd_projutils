...
в файле.
*/
	ushort  frameNum;       /* 64   Количество принятых строк. */
	ushort  lostFrameNum;   /* 66   Количество строк со сбоем синхронизации. */
/*
...
*/
    ushort  validTimeFrameNum;      /* 68   Количество строк, изначально принятых с правильным значением поля времени. */
    ushort  correctedTimeFrameNum;  /* 70   Количество строк, изначально принятых с неправильным значением поля времени, которое затем было исправлено. */
    ushort  gapNum;         /* 72   Количество пропусков данных (gaps). Общее количество кадров в этих дырах равно lostFrameNum. */

    ushort  packType;       /* 74   Тип упаковки. 0 - 1*10->2b, 1 - 3*10->4b, 2 - 8*10->10b */
    ushort  totalPixNum;    /* 76   Полная длина строки в пикселах. Для NOAA всегда 2048. */
    ulong   frameMask;      /* 78   Маска кадра HRPT. */
    ushort  pixGap;         /* 82   Количество пропущенных пикселов от начала строки. */
    ushort  pixNum;         /* 84   Количество принятых пикселов строки. */
    ushort  ascendFlag;     /* 86   Тип витка: 0 - нисходящий, 1 - восходящий. */
    uchar   reserved1[40];  /* 88 */

/* Телеграмма NORAD. */
    ulong   revNum;         /* 128  Номер опорного витка. */
    ushort  setNum;         /* 132  Номер набора. */
    ushort  ephemType;      /* 134  Тип эфемерид. */
    ushort  year;           /* 136  Год. */
...
*/
    ushort  corVersion;     /* 256  Номер версии коррекции, 0 - ее отсутствие. */
    short   corTBUSTime;    /* 258  Поправка времени, вычисленная по телеграмме TBUS (мсек). */
    short   corTime;        /* 260  Поправка времени, вычисленная иным способом (мсек). */
...
*/
    ushort  totalFrameNum;  /* 70 */
    ushort  totalPixNum;    /* 72 */
    ushort  pixGap;         /* 74 */
    ushort  pixNum;         /* 76 */
    ushort  ascendFlag;     /* 78 */
/*
    Поле maxPixelValue содержит максимальную величину среди ЗНАЧИМЫХ пикселов изображения.
*/
    ushort  maxPixelValue;  /* 80 */
/*
...
struct TBlk0_TLM {
    ...
    ushort  totalFrameNum;  /* 64 */
    ushort	channel;		/* 66 	Номер канала (1,2,3,4,5). */
...
struct TOldBlk0 {
        ...
        uchar sec;       /* 6 */
        uchar tic;       /* 7 */
        ushort revNum;      /* 8    Номер витка. */
        ushort totalScans;  /* 10   Число сканов в снимке. */
...
/*-------------------------------------------------------------------------*/
        ...
        ushort numberOfFrames;  /* 258 Количество малых кадров HRPT. */
        ushort frameLength;     /* 260 Длина кадра HRPT. */
        uchar unused11[2];       /* 262 */
        ushort frameSegMask;    /* 264 Маска сегментов кадра HRPT. */
        ushort pixStrPassed;    /* 266 Число пропущенных пикселов от начала строки AVHRR. */
        ushort pixStrLength;    /* 268 Число принятых пикселов строки AVHRR. */
/*-------------------------------------------------------------------------*/
        ushort projPixStrLength;    /* 270 Число пикселов в строке проекции. */
        ushort projNumberOfScans;   /* 272 Число строк проекции. */
        ushort projPixelBytes;      /* 274 Число байт на пиксел проекции. */
        ...
        ushort hProjShift;          /* 296 Сдвиг проекции по горизонтали. */
        ushort vProjShift;          /* 298 Сдвиг проекции по вертикали. */
        ushort processState;        /* 300 */
        float mKHR;                 /* 302 Среднее по фрагменту отношение высоты ИСЗ над Землей к радиусу орбиты. */
        ushort hPolyDegree;
        ushort vPolyDegree;
        float polyCoeffs[20];
/* Телеграмма NORAD. ----------------------------------------------------*/
        ushort nSatnum;     /* 390 номер спутника */
        ulong nRevnum;      /* 392 номер витка */
        ushort nElnum;      /* 396 номер набора элементов */
        ushort nEtype;      /* 398 тип эфемерид */
        ushort nYear;       /* 400 год */
        ushort nDay;        /* 402 день в году */
