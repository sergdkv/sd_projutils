...
/*
    Получение текущего имени файла как оно указано при вызове конфигуратора,
    или функции saveCfg.
    Если текущее имя не определено (использовался конструктор по умолчанию,
    а функция saveCfg вызвана не была) возвращается указатель на пустую строку.
*/
    const char * fullCfgPath() const { return fileName; }

/*
    Получение имени текущей конфигурации.
    Имя конфигурации --- это имя файла конфигурации, вместе с расширением.
*/
...
private:
    ...
    public:
        ...
        TCharString& operator[] (int i ) { return confString[i]; };
        void setIsKey(){ keyFlag = 1; };    // функция, вызываемая для установки флага
        int isKey(){ return keyFlag; };
	private:
