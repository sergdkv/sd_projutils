...
struct TStoreElement {
    double scan_n[3]; // Ќормаль плоскости сканировани€.
    double scan_d; // —вободный член уравнени€ плоскости сканировани€.
    double sat_pos[3];// ѕоложение спутника.
    double sub_p[3];// ѕодспутникова€ точка.
    double dir_v[3];// ¬ектор, перпендикул€рный плоскости радиус-вектора и вектора скорости. Ќужен дл€ определени€ знака угла.
    double sin_dov,cos_dov;// —инус и косинус угла на ќвна дл€ каждого скана.
};
...
{
    double tt, dov;
// “ранспонированна€ матрица пересчета координат из надирной с.к. в неподвижную геоцентрическую.
    double msi[3][3];
// “ранспонированна€ матрица перехода от с.к. сканера к неподвижной геоцентрической.
    double ma[3][3];
...
// »нициализаци€ орбитальной модели.
...
// ¬ычисление угла a.
            ...
            if( cosa > 1. ) cosa = 1.;  // этот код необходим из-за погрешностей машинных вычислений
            ...
            double mal[3][3] = {
                ...
                { 0., 1., 0. },
                { -sina, 0., cosa } };
// ¬ычисление матрицы ma, maT = msi * myp * malT * mr * ma.
            double mm1[3][3];
            for( m=0; m<3; m++){
                for( n=0; n<3; n++){
                    double t = 0.;
            ...
            // ¬ычисление подспутниковой точки.
            {
                double A,B,C,R,b[3];
    ...
    delete om;
    prev_lat = 100.;    // Ќевозможное значение.
}
...
int TInverseReferencer::ll2xy( double lon, double lat, double *x, double *y )
{
    ...
    u1 = 0;
    u2 = fTotalScans+1;
/* ѕереход к геоцентрической широте. */
    if( prev_lat != lat ){
        prev_lat = lat;
...
//--------------------------------------------------------------------------
// ѕристрелка перед дихотомией. ћы получаем u1 <= uu <= u2, где uu -
// истинный скан, причем u1 и u2 достаточно мало отличаютс€ от него.
//--------------------------------------------------------------------------
// »щем приближение дл€ u1.
    u1 += int(s1/(VR+1.1));
...
//--------------------------------------------------------------------------
// ÷икл точного определени€ скана при помощи дихотомического поиска.
//--------------------------------------------------------------------------
    while( u2 - u1 > 1 ){
        um = (u1 + u2) >> 1;
...
//--------------------------------------------------------------------------
// ќпределение позиции в скане.
//--------------------------------------------------------------------------
// ѕроецируем точку на плоскость сканировани€.
...
// ќпредел€ем угол.
    ...
    if( xx > 1024. ) return 0;
//  смотрим, в какую сторону от направлени€ на подспутниковую точку надо откладывать угол
//  при этом надир проходит через точку 1024.0, центром первого пиксела скана €вл€етс€ точка 0.5,
//  центром последнего пиксела скана €вл€етс€ точка 2047.5
