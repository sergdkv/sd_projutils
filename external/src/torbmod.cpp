...
// Отслеживание перехода через границу года. Полагаем, что коррекция по времени имеет величину порядка секунды.
        ...
        else{   // Переход через границу года вперед.
...
void TOrbitalModel::ini_sgp8()
{
    ...
    i = isp.i0; // в модели SGP8 наклонение орбиты считается постоянным
...
void TOrbitalModel::sgp8( double rel_time )
{
    ...
    w = dinpi(isp.w0 + wf*t1 + ws*tt);// Аргумент перигея.
    m = dinpi(isp.m0 + n0*tt + mf*t1 + ms*tt + t2);// Средняя аномалия.
    }
